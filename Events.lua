AstralRaidEvents = CreateFrame('FRAME', 'AstralRaidEvents')
AstralRaidEvents.dtbl = {}
...
function AstralRaidEvents:Unregister(event, name)
...
function AstralRaidEvents:OnEvent(event, ...)
	local objs = self.dtbl[event]
	if not objs then return end
...
AstralRaidEvents:SetScript('OnEvent', AstralRaidEvents.OnEvent)